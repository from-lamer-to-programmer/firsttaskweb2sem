...
if (!preg_match('/^[а-яёА-ЯЁa-zA-Z\sы]+$/', $name)) {
    echo "Ошибка: Поле ФИО должно содержать только буквы русского и английского алфавита, пробелы и символ 'ы'";
    exit;
}
...
if (!preg_match('/^[0-9+]+$/', $number)) {
    echo "Ошибка: Номер телефона должен содержать только цифры и символ '+'";
    exit;
}
